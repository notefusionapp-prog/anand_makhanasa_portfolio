...
import { ALL_PROJECTS, PUBLISHED_APPS, WEB_PROJECTS, Project } from '../data/portfolioData';
import { SEOHead } from '../components/SEOHead';
import { OptimizedProjectImage } from '../components/OptimizedProjectImage';

export const ProjectsPage: React.FC = () => {
  const [selectedFilter, setSelectedFilter] = useState<'all' | 'published-mobile' | 'web-admin' | 'websites' | 'productivity'>('all');

  const filteredApps = ALL_PROJECTS.filter((app) => {
    if (selectedFilter === 'all') return true;
    if (selectedFilter === 'published-mobile') return app.type === 'published' && app.projectType === 'mobile-app';
    if (selectedFilter === 'web-admin') return app.projectType === 'flutter-web' || app.projectType === 'admin-panel';
    if (selectedFilter === 'websites') return app.projectType === 'website';
    if (selectedFilter === 'productivity') return app.category === 'productivity';
    return true;
  });
  ...
  const projectsCollectionSchema = {
    ...
    "mainEntity": {
      "@type": "ItemList",
      "itemListElement": ALL_PROJECTS.map((app, index) => ({
        "@type": "ListItem",
        "position": index + 1,
        "item": {
    ...
    <div className="pt-28 pb-20 min-h-screen bg-[#fbfdff] dark:bg-[#070d1e] text-[#0B1B3D] dark:text-slate-100 transition-colors duration-300">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <nav aria-label="Breadcrumb" className="mb-8 flex items-center gap-2 text-xs text-slate-500 dark:text-slate-400 font-medium">
          ...
          <span>/</span>
          <span className="text-[#0B1B3D] dark:text-white font-semibold">Projects &amp; Applications</span>
        </nav>
        ...
        <div className="text-center max-w-3xl mx-auto mb-12">
          ...
          <div className="mt-8 flex flex-wrap justify-center gap-2">
            ...
            <button
              ...
              className={`px-4 py-2 rounded-full text-xs font-semibold transition-all cursor-pointer ${
                selectedFilter === 'published-mobile'
                  ? 'bg-[#0d6efd] text-white shadow-md shadow-blue-500/20'
                  : 'bg-white dark:bg-[#0B1B3D] border border-blue-100 dark:border-blue-900 text-slate-600 dark:text-slate-300 hover:border-blue-300'
              }`}
            >
            ...
            >
              ...
              <div className="p-6">
                <div className="flex items-start gap-3.5 mb-4">
                  {app.iconUrl && (
                    <div className="w-14 h-14 rounded-2xl overflow-hidden border border-slate-200 dark:border-slate-700 shadow-sm shrink-0 bg-slate-900/10">
                  ...
                  <div>
                    <div className="text-[11px] font-bold text-[#0d6efd] dark:text-blue-400 uppercase tracking-wider">
                      {app.projectType === 'flutter-web' ? 'Flutter Web' : app.projectType === 'website' ? 'SEO Website' : app.category || 'Mobile App'} • {app.role}
                    </div>
                ...
                {app.screenshots && app.screenshots[0] && (
                  <div className={`rounded-2xl overflow-hidden bg-black/90 relative border border-slate-200 dark:border-slate-800 flex items-center justify-center ${app.deviceType === 'browser' ? 'h-48' : 'h-44'}`}>
                    <OptimizedProjectImage 
                ...
                <div className="flex items-center gap-2">
                  {app.liveUrl && (
                    <a
                      ...
                      className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-[#10b981] hover:bg-[#059669] text-white text-[11px] font-bold shadow-xs transition-colors"
                    >
                      <span>Live Web</span>
